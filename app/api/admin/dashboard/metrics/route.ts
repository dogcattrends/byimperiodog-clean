...
function deriveSanityPostStatus(publishedAt?: string | null, status?: string | null) {
...
export async function GET(req: NextRequest) {
 ...
 `*[_type == "post"]
 | order(coalesce(_createdAt, _updatedAt) desc)[0...2000]{
 _id,
 slug,
 title,
 description,
 ...
 const postsArr = (sanityPosts || []).map((p): PostRow => {
 const category =
 p.category ||
 (Array.isArray(p.categories) ? (p.categories[0]?.slug || p.categories[0]?.title || null) : null);
 const status = deriveSanityPostStatus(p.publishedAt ?? null, p.status ?? null);
 return {
 id: p._id,
 slug: p.slug?.current || p._id,
 title: p.title || 'Post',
 status,
 created_at: p._createdAt ?? null,
 published_at: p.publishedAt ?? null,
 scheduled_at: status === 'scheduled' ? (p.publishedAt ?? null) : null,
 author_id: p.author?._id ?? null,
 category: category ? String(category).toLowerCase() : null,
 seo_title: p.title ?? null,
 seo_description: p.description ?? null,
 cover_url: p.coverUrl ?? p.coverImage?.asset?.url ?? p.mainImage?.asset?.url ?? null,
 og_image_url: p.coverUrl ?? p.coverImage?.asset?.url ?? p.mainImage?.asset?.url ?? null,
 };
 });
 ...
 if (authorIds.length) {
 const authorRows = await sanityClient.fetch<Array<{ id: string; name: string; avatar_url?: string | null }>>(
 `*[_type == "author" && _id in $ids]{
 "id": _id,
 name,
 "avatar_url": image.asset->url
 }`,
 { ids: authorIds }
 );
 ...
 .select("id,post_id,post_slug,run_at,action,payload,executed_at")
 .is("executed_at", null)
 ...
 ? String((event.payload as any).post_slug ?? (event.payload as any).slug ?? '').trim()
 : '';
 const effectiveSlug = (event.post_slug && String(event.post_slug).trim()) ? String(event.post_slug).trim() : (slugFromPayload || null);
 return {
 id: event.id,
 post_id: event.post_id,
 run_at: event.run_at,
 action: event.action,
 post_slug: effectiveSlug,
 post_title: (effectiveSlug ? postsArr.find((post) => post.slug === effectiveSlug)?.title : null) || (event.post_id ? postsArr.find((post) => post.id === event.post_id)?.title : null) || null,
 };
 }) || [];

 const { data: commentsData } = await supa
 .from("blog_comments")
 .select("id,post_id,post_slug,author_name,body,created_at")
 .eq("approved", false)
 .order("created_at", { ascending: false })
 .limit(MAX_LIST_ITEMS);
 ...
 post_slug: comment.post_slug ?? null,
 author_name: comment.author_name,
 excerpt: (comment.body || '').slice(0, 140),
 created_at: comment.created_at,
 post_title: (comment.post_slug ? postsArr.find((post) => post.slug === comment.post_slug)?.title : null) || (comment.post_id ? postsArr.find((post) => post.id === comment.post_id)?.title : null) || null,
 })) || [];
