...
export async function POST(req: Request) {
 ...
 try {
 const { slug, insertedId, cover_url } = await persistPost({ title, excerpt, mdx, seo_title, seo_description, tags, status: body.status, scheduled_at: body.scheduled_at, generateImage: body.generateImage, coverPrompt: `Foto realista 16:9 filhote Spitz Alemão Lulu da Pomerânia ${topic}`, coverAlt: `Filhote Spitz Alemão Lulu da Pomerânia - ${topic}`, extras: { contentBlocks, readingTime, faqItems } });
...
function buildTitle(topic: string, scope: string): string {
 ...
 if (scope === 'filhote') return `Guia Filhote de Spitz Alemão Lulu da Pomerânia: ${clean}`;
 if (scope === 'adulto') return `Spitz Alemão Lulu da Pomerânia Adulto: ${clean}`;
 return `Guia Completo Spitz Alemão Lulu da Pomerânia: ${clean}`;
}

function buildExcerpt(mdx: string, topic: string): string {
 const plain = mdx.replace(/[#>*`_-]/g,' ').replace(/\s+/g,' ').trim();
 return (plain.slice(0, 155) || `Tudo sobre ${topic} no contexto do Spitz Alemão Lulu da Pomerânia.`);
}
...
function sanitizeTopic(t: string){
 return t.replace(/["'`]+/g,'').replace(/\b(assunto do artigo)\b/gi,'Spitz Alemão Lulu da Pomerânia');
}

function refineMDX(mdx: string, topic: string){
 let out = mdx;
 // Remover citações literais exageradas do título
 out = out.replace(/Guia Completo Spitz Alemão Lulu da Pomerânia:\s+"([^"]+)"/gi,(m,inner)=>`Guia Completo Spitz Alemão Lulu da Pomerânia: ${inner}`);
...
function ensureFAQMinimum(mdx: string): string {
 ...
 const samples = [
 { q: 'Spitz Alemão Lulu da Pomerânia late muito?', a: 'Pode vocalizar; treino de reforço positivo ajuda a reduzir.' },
 ...
 async function persistPost(opts: { title: string; excerpt: string; mdx: string; seo_title: string; seo_description: string; tags: string[]; status?: string; scheduled_at?: string|null; generateImage?: boolean; coverPrompt: string; coverAlt: string; extras?: PersistExtras; }) {
 const { title, excerpt, mdx, seo_title, seo_description, tags, status, scheduled_at, generateImage, coverPrompt, coverAlt, extras } = opts;
 const slug = slugify(title);

 let inserted: { id?: string; slug?: string } | null = null;
 let lastErr: unknown = null;
 const desiredStatus = (status || 'draft') as 'draft'|'review'|'scheduled'|'published'|'archived';
 const publishedAt = desiredStatus === 'published' ? new Date().toISOString() : (desiredStatus === 'scheduled' ? (scheduled_at || null) : null);
 const contentPlain = toPlain(mdx);
...
async function ensureMinWords(mdx: string, min: number, openaiKey: string|undefined, temperature: number, topic: string): Promise<string> {
 ...
 const filler = `\n\n## Detalhamento Adicional\n${Array.from({length: Math.ceil(deficit/120)}).map((_,i)=>`Parágrafo de aprofundamento (${i+1}) sobre ${topic}, cobrindo aspectos práticos, exemplos reais e dicas aplicáveis para tutores de Spitz Alemão Lulu da Pomerânia.`).join('\n\n')}`;
