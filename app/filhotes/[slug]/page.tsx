...
import { buildQualifiedWhatsAppMessage, buildWhatsAppLink } from "@/lib/whatsapp";
...
export default async function PuppyDetailPage({ params }: Props) {
 ...
 const canonicalUrl = `${siteUrl}/filhotes/${puppy.slug}`;
 ...
 message: buildQualifiedWhatsAppMessage({
  ...
  puppySex: translateSex(puppy.sex),
  puppyLocation: [puppy.city, puppy.state].filter(Boolean).join(" / ") || null,
 }),
 utmSource: "site",
