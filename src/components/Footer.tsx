...
const buildFooterWhatsapp = (origin: string) =>
 buildWhatsAppLink({
    message: `Olá! Vim pelo ${origin} do site By Império Dog e quero falar sobre filhotes disponíveis.`,
  utmSource: "site",
  utmMedium: "footer",
...
const WA_CTA = buildFooterWhatsapp("footer-cta");
const WA_NAV = buildFooterWhatsapp("footer-nav");
const WA_CONTACT = buildFooterWhatsapp("footer-contact");
const WA_FLOATING = buildFooterWhatsapp("footer-floating");
...
const menuLinks: { label: string; path: AppRoutes }[] = [
 ...
 { label: "Comprar", path: routes.comprar },
 { label: "Preço", path: routes.preco },
 { label: "Criador confiável", path: routes.criador },
...
export default function FooterFixed() {
 ...
 Pronto para garantir seu Spitz Alemão Lulu da Pomerânia?
 </h2>
 ...
 Spitz Alemão Anão Lulu da Pomerânia, criado com responsabilidade, saúde validada e mentoria vitalícia. Atendimento premium — consulte valores para famílias que valorizam transparência.
 </p>
 <a
 href={WA_NAV}
 target="_blank"
 ...
 <p className="font-medium text-zinc-300">&copy; {year} By ImpÃ©rio Dog</p>
