...
export default function PuppyCatalogCard({ puppy }: Props) {
  ...
  const cardTitle = useMemo(() => {
    ...
    if (pieces.length === 0) return "Spitz Alemão Anão";
    return `Spitz Alemão Anão – ${pieces.join(" ")}`;
  }, [sexLabel, colorLabel]);

  const primaryLabel = "Ver detalhes e conversar";
  ...
  const handleCTA = (event?: MouseEvent<HTMLButtonElement>) => {
    ...
    const payload = getTrackingContext({ placement: "catalog_card", action: "cta_primary" });
  ...
  return (
    <>
      <article
        ...
        aria-label={`Abrir detalhes do filhote — ${status?.label ?? "Disponível"}`}
      >
        {/* CAPA */}
        <div className="relative">
          <div className="relative w-full h-[210px] sm:h-[240px] md:h-[280px] overflow-hidden rounded-t-3xl bg-zinc-50">
            {coverMedia.kind === "video" ? (
              ...
              <Image
                src={coverMedia.url}
                alt={`${displayName} — filhote de Spitz Alemão`}
                fill
        ...
        <div className="flex flex-1 flex-col gap-3 px-5 pb-5 pt-4">
          <div>
            <h3 className="text-lg font-semibold text-zinc-900 line-clamp-2">{cardTitle}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {sexLabel && (
              <span className="rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-600">{sexLabel}</span>
            )}
            {colorLabel && (
          ...
          <div className="mt-auto flex flex-wrap gap-3">
            <button
              ...
              type="button"
              onClick={handleCTA}
              aria-label={`Ver detalhes e conversar sobre: ${displayName}`}
