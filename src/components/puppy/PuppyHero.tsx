 ...
 * UX: Clareza imediata sobre disponibilidade e prÃ³ximo passo
 * UX: Clareza imediata sobre disponibilidade e próximo passo
 * A11y: h1 semÃ¢ntico, aria-labels, foco visÃ­vel
 * 
 * Migrado para usar componentes do Design System:
 * - Button para CTAs
 * - StatusBadge para status
 * - Card para descriÃ§Ã£o (opcional)
 */
...
export function PuppyHero({ puppy, whatsappLink, onFavorite, isFavorited }: Props) {
 ...
 alt={`Filhote ${puppy?.name ?? "Filhote"} - Spitz Alemão Anão Lulu da Pomerânia ${puppy?.color ?? ""}, ${translateSex(puppy?.sex)}, ${location}`}
...
Imagem não disponível
 ...
 {/* InformaÃ§Ãµes principais e CTA */}
 ...
 aria-label="Quero esse filhote - Atendimento via WhatsApp"
 className="w-full"
 >
 <Button
 variant="solid"
 ...
 <p className="text-center text-xs text-zinc-500">Resposta em até 1 hora • Atendimento humanizado 7 dias por semana</p>
 </div>
 </div>
 </section>
 );
