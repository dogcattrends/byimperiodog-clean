...
const GUIDE_VERSION_OBJECT_PATHS: Record<string, string> = {
 // Caminho dentro do bucket (nao URL). Mantem o download protegido via /download/guia.
 v1: process.env.GUIDE_STORAGE_V1_OBJECT_PATH || "guides/guia.pdf",
 v2: process.env.GUIDE_STORAGE_V2_OBJECT_PATH || "guides/guia-v2.pdf",
};

const CURRENT_VERSION = process.env.GUIDE_CURRENT_VERSION || "v1";
const DEFAULT_VERSION = GUIDE_VERSION_OBJECT_PATHS[CURRENT_VERSION] ? CURRENT_VERSION : "v1";
...
function resolveVersion(proposed?: string) {
 if (proposed && GUIDE_VERSION_OBJECT_PATHS[proposed]) return proposed;
 if (GUIDE_VERSION_OBJECT_PATHS[DEFAULT_VERSION]) return DEFAULT_VERSION;
 return "v1";
}

export function getGuideStorageObjectPath(version?: string) {
 const key = resolveVersion(version);
 return GUIDE_VERSION_OBJECT_PATHS[key] ?? GUIDE_VERSION_OBJECT_PATHS.v1;
}
