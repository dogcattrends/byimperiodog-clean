...
export const scheduleInputSchema = z.object({
 postSlug: trim(z.string().min(3).max(160)).optional().nullable(),
 // Compat (legado): UUID do Supabase. Preferir postSlug.
 postId: z.string().uuid().optional().nullable(),
 runAt: z.string().datetime(),
 repeatInterval: z.number().int().positive().optional().nullable(),
}).superRefine((data, ctx) => {
 const slug = typeof data.postSlug === 'string' ? data.postSlug.trim() : '';
 const hasSlug = Boolean(slug);
 const hasId = Boolean(data.postId);
 if (!hasSlug && !hasId) {
 ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Informe postSlug (preferido) ou postId (legado).' });
 return;
 }
 if (hasSlug && !/^[a-z0-9-]+$/.test(slug)) {
 ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'postSlug inválido. Use apenas letras minúsculas, números e hífen.' });
 }
});
