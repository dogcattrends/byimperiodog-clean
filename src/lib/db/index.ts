...
export const blogRevisionsRepo = {
...
export const scheduleRepo = {
 async listEvents(params?: { postSlug?: string; postId?: string; limit?: number }): Promise<ScheduleEvent[]> {
 const client = getClient();
 if (!client) return [];
 try {
 let query = client
 .from("blog_post_schedule_events")
 .select("id,post_id,post_slug,status,run_at,finished_at,attempts,created_at,updated_at")
 .order("run_at", { ascending: true });
 if (params?.postSlug) query = query.eq("post_slug", params.postSlug);
 else if (params?.postId) query = query.eq("post_id", params.postId);
 if (typeof params?.limit === "number") query = query.limit(params.limit);
 ...
 const { postId, postSlug, runAt, repeatInterval } = parsed.data;
 const resolvedSlug = typeof postSlug === 'string' ? postSlug.trim() : '';
 try {
 const { data, error } = await client
 .from("blog_post_schedule_events")
 .insert({
 post_id: postId ?? null,
 post_slug: resolvedSlug || null,
 run_at: runAt,
