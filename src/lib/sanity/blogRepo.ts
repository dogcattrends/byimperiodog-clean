...
function mapDocumentToPost(doc: SanityPostDocument): Post {
 ...
 ogImageUrl:
 doc.ogImageUrl ?? doc.coverUrl ?? doc.coverImage?.asset?.url ?? doc.mainImage?.asset?.url ?? null,
 score: null,
 },
...
function buildDocument(data: PostContentInput, authorId: string, docId: string) {
 ...
 canonicalUrl: (data as unknown as { canonicalUrl?: string | null }).canonicalUrl ?? null,
 robots: (data as unknown as { robots?: string | null }).robots ?? null,
 ...
 const search = rawSearch && rawSearch.length >= 2 ? `${rawSearch}*` : undefined;
    const searchClause =
     search && search.length > 2
        ...
        : "";
    const finalFilter = `*[_type == "post"${statusClause}${categoryClause}${tagClause}${searchClause}]`;
