...
export const TOPIC_CLUSTERS: TopicCluster[] = [
  ...
  { key: 'checklist-adocao', title: 'Checklist Adoção/Compra', keywords: ['checklist','adocao','adoção','compra'] },
];
...
export function computeCoverage(posts: { slug?: string; title?: string; status?: string }[]) {
  ...
  return { total, covered, percent, missing: TOPIC_CLUSTERS.filter(c=> !coveredKeys.has(c.key)).map(c=>c.title) };
}
