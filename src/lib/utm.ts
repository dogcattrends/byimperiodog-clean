import { buildQualifiedWhatsAppMessage } from "@/lib/whatsapp";
...
export function whatsappLeadUrl(phoneE164: string, context: LeadContext) {
 ...
 const originParts = [
  context.pageType,
  context.slug ? `/${context.slug}` : null,
  context.color ? `cor=${context.color}` : null,
  context.city ? `cidade=${context.city}` : null,
 ].filter(Boolean);
 const origin = originParts.join(" ");
 const text = buildQualifiedWhatsAppMessage({
  origin,
