...
type Tables = { seo_suggestions: Row[] };
let table: Tables = { seo_suggestions: [] };
...
vi.mock('../src/lib/sanity/client', () => ({
 ...
 // applySuggestion resolve docId por slug
 if (query.includes('slug.current') && query.includes('[0]._id')) {
 return params?.slug ? `doc_${params.slug}` : null;
 }
 return null;
 }),
 patch: vi.fn(() => {
...
beforeEach(()=>{ table = { seo_suggestions: [
 { id:'s1', entity_type:'blog_post', entity_id:null, entity_ref:'s1', data_json:{ seoTitle:'A', seoDescription:'Desc A'}, score:10, status:'proposed' },
 { id:'s2', entity_type:'blog_post', entity_id:null, entity_ref:'s2', data_json:{ seoTitle:'B'}, score:5, status:'approved' },
] }; });
