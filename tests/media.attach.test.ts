...
let scenario: 'media-missing'|'cover-success'|'gallery-success' = 'cover-success';

vi.mock('@/lib/sanity', () => ({
 sanityClient: {
 fetch: vi.fn(async (query: string) => {
 // resolve postId via slug
 if (query.includes('slug.current') && query.includes('[0]._id')) return 'post1';
 return null;
 }),
 patch: vi.fn(() => ({ set: () => ({ commit: () => Promise.resolve() }) }))
 }
}));
