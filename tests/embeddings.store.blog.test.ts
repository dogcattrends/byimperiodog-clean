...
interface EmbRow { post_slug:string; source:string; embedding:string }
const table: EmbRow[] = [];
let failEmbed = false;
...
vi.mock('../src/lib/supabaseAdmin', () => ({
 ...
 return {
 upsert: async (row: { post_slug:string; source:string; embedding:string }) => {
 const idx = table.findIndex(r=> r.post_slug===row.post_slug && r.source===row.source);
...
describe('embeddings.store.blog', () => {
 ...
 const ok = await ensurePostEmbedding({ slug:'p1', title:'Titulo', content:'Conteúdo **markdown**' });
 expect(ok).toBe(true);
 ...
 failEmbed = true;
 const ok = await ensurePostEmbedding({ slug:'p2', title:'Falha', content:'Texto' });
 expect(ok).toBe(false);
 expect(table.length).toBe(0);
 });

 it('batchEnsureEmbeddings processa subset e agrega resultados', async () => {
 const posts = Array.from({ length:5 }).map((_,i)=> ({ slug:`b${i+1}`, title:`T${i+1}`, content:'x'.repeat(10+i) }));
 ...
 const posts = [ { slug:'c1', title:'ok', content:'aaa' }, { slug:'c2', title:'err', content:'bbb' } ];
