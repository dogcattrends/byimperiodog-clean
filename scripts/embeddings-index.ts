...
async function upsertEmbeddings(rows: { id: string; slug: string; embedding: number[]; source: string }[]) {
 const payload = rows.map((r) => ({ post_slug: r.slug, source: r.source, embedding: JSON.stringify(r.embedding) }));
 const { error } = await supa.from('blog_post_embeddings').upsert(payload, { onConflict: 'post_slug,source' });
...
async function run(){
 ...
 if(rows.length) await upsertEmbeddings(rows);
 console.log('Concluido. Vetores (upsert):', rows.length);
}
