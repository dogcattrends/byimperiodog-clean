...
- **`/guia` (app/guia/page.tsx)** — landing do lead magnet com formulário que chama `/api/leads` e exibe CTA de download (`public/guia.pdf`). **Risco:** vazamento de PDF sem controle de tokens; formulário precisa LGPD. **Oportunidade:** reorganizar funil e tracking `lead_submit`, `pdf_download`.
