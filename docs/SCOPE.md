...
- Telemetria e funil são tratados por um “single source” de tracking (`src/lib/track.ts` + `app/api/tracking/settings/route.ts` + `scripts/seo-audit.ts`), mantendo rastro de eventos `cta_click`, `modal_open`, `lead_submit`, `pdf_downloaded` e `page_view` conforme consentimento do usuário. CTA/Contato seguem componentes únicos (`src/components/ui/PrimaryCTA.tsx`, `src/components/ui/ContactCTA.tsx`, `src/components/ui/AccessibleModal.tsx`) para foco, estilo e métricas consistentes.
...
- **Guia (`app/guia/page.tsx`)**: lead magnet com formulário mínimo (nome, WhatsApp, email opcional, consentimento) que gera tokens em `lead_download_tokens`, confirma o envio e entrega o PDF seguro via `/download/guia?token=`; validado com `aria-describedby` e tracking `lead_submit`/`pdf_downloaded`.
