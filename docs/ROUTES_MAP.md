...
| `/blog/[slug]` | Post com TL;DR, TOC, FAQ, JSON-LD (Article/Breadcrumb/FAQ), relacionados por tag (`app/blog/[slug]/page.tsx`). | `Ver filhotes` + `Baixar guia` | Sanity (`src/lib/sanity/client.ts`) + fallback Contentlayer quando aplicável. | Schema, canonical e OG image (dynamic) devem ser absolutos; share/related mantêm performance |
